...
for id in jobId:
    ...
    #requisitionList = list(db.requisition.find({"new_global_job_category_id": id,"pre_identified_req": False,"req_status_id" : {"$in":[2,3,6,9,10,11,12]}},{"requisition_id":1,"_id":0}))

    #requisitionList = list(db.requisition.find({"new_global_job_category_id": id,"pre_identified_req": False,},{"requisition_id":1,"_id":0}))

    ## - Change the client ID's for client who are running Zindex Scoring
    requisitionList = list(db.requisition.find({"new_global_job_category_id": id,"client_id":{"$in":[344,345,3307,513,514,734]},"pre_identified_req": False,"req_status_id" : {"$in":[2,3,6,9,10,11,12]}},{"requisition_id":1,"_id":0}))
