...
def insertIdealCharacteristics(idealskills, jobcatid, jobclassid, reqid, clientid, datacenter):
    idealtable = {}
    ...
    db.client_ideal_candidate_characteristics_test.insert_one(idealtable)
...
def ideal_characteristics_build(reqid):
    ...
    #    reqid.append(requisition["requisition_id"])
    ...
    if(len(hiredCandidates)==0):
        insertIdealCharacteristics("NA", jobcatid, jobclassid, reqid,clientid,datacenter)
    ...
    insertIdealCharacteristics(idealskills, jobcatid, jobclassid, reqid,clientid,datacenter)
    return("Success")
...
for requisition in requisitionList:
    ...
    exist = db.client_ideal_candidate_characteristics_test.count({"requisition_id":requisition})
    if(exist==1):
