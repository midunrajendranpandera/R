...
requisitionList = [38614]

#resReqList = list(db.requisition_skills_from_parsed_requisition.find({}).distinct("requisition_id"))
#commonReqList = list(set(resReqList).intersection(set(requisitionList)))
...
for requisition in requisitionList:
    ...
    for candidate in candNoResume:
        ...
        db.requisition_cand_zindex_scores_temp.update(key,zindex_score,upsert=True)
    ...
    for cand_resume_parsed in candResumeParsed:
        ...
        if(resWordsLength == 0):
            ...
            zindex_distribution = [zindex_skill_score,zindex_exp_score,zindex_jobfit_score]
            zindex_score["zindex_distribution"] = zindex_distribution
            print("Requisition %s Candidate %s Inserting Zeroes" %(requisition,zindex_score["candidate_id"] ))
            db.requisition_cand_zindex_scores_temp.update(key,zindex_score,upsert=True)
            continue
        ...
        db.requisition_cand_zindex_scores_temp.update(key,zindex_score,upsert=True)
    print("Requisition Time %s" %(time.time() - reqTime))
#elapsedTime = time.time()-startTime
print("Total time elapsed %s" %(time.time() - startTime))
