...
def insertIdealCharacteristics(idealskills, gjobcatid, reqidList):
    ...
    db.ideal_candidate_characteritics_temp2.update(key,idealtable,upsert=True)

def ideal_characteristics_build(gjobcatid):
    #Retreive Requisition Details
    #reqDetails = list(db.requisition.find({"requisition_id": reqid},{"requisition_id": 1, "global_job_category_id": 1, "data_center":1, "_id": 0}))
    #for requisition in reqDetails:
        #gjobcatid = requisition["global_job_category_id"]
        #datacenter = requisition["data_center"]
    # Find relevant requisitions
    relevantRequisitions = list(db.requisition.find({"global_job_category_id": gjobcatid},{"requisition_id": 1, "_id": 0}))
    reqidList = []
...
globalJobCatList = list(db.requisition.find({}).distinct("global_job_category_id"))
#requisitionList = [5]
...
for jobid in globalJobCatList:
    ...
    exist = db.ideal_candidate_characteritics_temp2.count({"global_job_category_id":jobid})
    if(exist==1):
