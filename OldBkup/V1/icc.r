###############################################################################################
#      R function to create ideal characteristics table for Zero Chaos requisitions           #
###############################################################################################
...
icc <- function(ReqId)
{
	...
	if(CandidateList=="No Relevant Profile Available"){
		...
		data<-list(requisition_id=ReqId,global_job_category_id=jobid,Region=data_center,Skills="No Skills")
		data<-toJSON(data)
		output <- dbInsertDocument(mongo, idcc, data)
	...
	if(length(queryout)==0){
		...
		data<-list(requisition_id=ReqId,global_job_category_id=jobid,Region=data_center,Skills="NA")
		data<-toJSON(data)
		output <- dbInsertDocument(mongo, idcc, data)
	...
	data<-list(requisition_id=ReqId,global_job_category_id=jobid,Region=data_center,Skills=Skills)
	data<-toJSON(data)
	output <- dbInsertDocument(mongo, idcc, data)
	dbDisconnect(mongo)
	return("Success")
