...
for(m in 1:length(jobid)){
	...
	openreq<-integer()
	leng<-length(reqlist)
	totalreqcount<-totalreqcount+leng
	...
	openreqcount<-openreqcount+opnreqleng
	print(paste(paste("Job Id - ",jobid[m]),paste("Open Requisition - ",opnreqleng)))
}
print(paste(paste("Total Requisitions - ",totalreqcount),paste("Open Requisitions - ",openreqcount)))
