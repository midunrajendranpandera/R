...
reqscoring<-function(){
	...
	reqidlist<-dbGetDistinct(mongo, sscc, "requisition_id")
	requisitionlist<-setdiff(reqlist,reqidlist)
	...
	queryout<-queryout$requisition_id
	queryout<-unique(queryout)
	requisitionlist <- intersect(requisitionlist,queryout)
	if(length(requisitionlist)==0){
		print("No New Requisition")
		return("No New Requisition")
	}
	 
	l<-length(requisitionlist)
	print(paste("Total New Requisition",l))
	for(i in 1:l){
		print(paste("Working on Requisition - ",requisitionlist[i]))
		reqjson<-list(requisition_id=requisitionlist[i])
		reqjson<-toJSON(reqjson)
		...
		reqjson<-list(requisition_id=as.integer(requisitionlist[i]))
		reqjson<-toJSON(reqjson)
		keys<-list("Skills"=1)
		keys<-append(keys,list("_id"=0))
		keys<-toJSON(keys)
	...
	state<-paste("New Requisition # - ",reqlen)
	state<-paste(state,"are scored for their corresponding job id's")
	return(state)
}
status<-reqscoring()
print(status)
